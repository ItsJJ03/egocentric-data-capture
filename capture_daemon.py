#!/usr/bin/env python3
"""
Capture Daemon — entry point, runs on Pi boot as systemd service.
...
# Make capture package importable from repo root
...
def gpio_loop():
    ...
    while not _daemon_stop.is_set():
        ...
        if prev_start == 1 and cur_start == 0 and (now - last_start) > DEBOUNCE_S:
            last_start = now
            log.info("Button 2 pressed → start recording")
...
def shutdown(signum, frame):
    ...
    _daemon_stop.set()
    if GPIO_OK: lgpio.gpiochip_close(h)
    sys.exit(0)
...
def main():
    ...
    log.info("=" * 50)
    log.info("Egocentric Capture Daemon starting")
