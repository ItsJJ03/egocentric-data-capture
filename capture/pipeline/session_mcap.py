"""
MCAP session — coordinates Orbbec bag + 2x Kreo MP4.
Post-processing: bag → MCAP (no combined grid, no color/depth MP4).

Outputs per session:
  orbbec_<id>.bag       — raw Orbbec recording
  orbbec_<id>.mcap      — Foxglove-compatible (color JPEG + depth uint16)
  kreo1_<id>.mp4        — left wrist
  kreo2_<id>.mp4        — right wrist
  timestamps_<id>.csv   — Kreo frame timestamps
  session_<id>.txt      — manifest
"""
...
class McapCaptureSession:
    ...
    def _state(self, state: str, detail: str = ""):
        log.info(f"[mcap-session] {state}: {detail}")
    ...
    def _run(self):
        ...
        self._state('converting', "Converting bag → MCAP...")
        mcap_ok = convert_bag_to_mcap(bag_out, mcap_out)

        self.files = {
            'bag':        bag_out,
            'mcap':       mcap_out  if mcap_ok  else None,
